...
@section('title', 'Stock Register - Heavy Loom Material Ledger')
...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
    <div>
        <h1 style="font-size: 24px; font-weight: 700; color: var(--text-color); margin: 0; display: flex; align-items: center; gap: 10px;">
            <span class="material-symbols-outlined" style="color: var(--primary-color); font-size: 28px;">swap_vert</span>
            Stock Register
        </h1>
        <p style="color: var(--text-muted); margin: 4px 0 0 0; font-size: 13px;">Owner ledger tracking yarn/materials issued to workers and finished material received back from workers.</p>
    </div>
...
<div class="metrics-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 24px;">
    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Total Material Given</span>
        ...
        <div style="font-size: 24px; font-weight: 800; color: #f59e0b; margin-top: 10px;" id="metricQtyIn">
            {{ number_format($totalQtyIn, 3) }} <span style="font-size: 14px; font-weight: 600;">kg</span>
        </div>
    </div>

    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Total Material Received</span>
        ...
        <div style="font-size: 24px; font-weight: 800; color: #10b981; margin-top: 10px;" id="metricQtyOut">
            {{ number_format($totalQtyOut, 3) }} <span style="font-size: 14px; font-weight: 600;">kg</span>
        </div>
    </div>

    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Material Balance with Worker</span>
        ...
        <div style="font-size: 24px; font-weight: 800; color: #3b82f6; margin-top: 10px;" id="metricBalance">
            {{ number_format(abs($currentBalance), 3) }} <span style="font-size: 14px; font-weight: 600;">kg</span>
        </div>
    </div>

    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Total Outgoing Cones</span>
            <span class="material-symbols-outlined" style="color: #8b5cf6; background: rgba(139, 92, 246, 0.1); padding: 6px; border-radius: 8px;">category</span>
        </div>
        <div style="font-size: 24px; font-weight: 800; color: #8b5cf6; margin-top: 10px;" id="metricCones">
            {{ number_format($totalCones) }} <span style="font-size: 14px; font-weight: 600;">cones</span>
        </div>
    </div>

    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Total Stock Records</span>
            <span class="material-symbols-outlined" style="color: #ec4899; background: rgba(236, 72, 153, 0.1); padding: 6px; border-radius: 8px;">receipt_long</span>
        </div>
        <div style="font-size: 24px; font-weight: 800; color: var(--text-color); margin-top: 10px;">
            {{ $entries->total() }} <span style="font-size: 14px; font-weight: 500; color: var(--text-muted);">entries</span>
        </div>
    </div>
</div>
...
<div class="card" style="padding: 16px 20px; border-radius: 12px; margin-bottom: 20px; background: var(--bg-surface); border: 1px solid var(--border-color);">
    <form method="GET" action="{{ route('stock-register.index') }}" autocomplete="off" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
        ...
        <div style="flex: 1; min-width: 180px;">
            <label style="display: block; font-size: 12px; font-weight: 600; color: var(--text-muted); margin-bottom: 6px;">SEARCH ITEM / DETAILS</label>
            <input type="text" name="search" class="form-control" autocomplete="off" value="{{ request('search') }}" placeholder="Search by quality, worker or notes..." style="width: 100%;">
        </div>
        ...
        <div style="min-width: 140px;">
            ...
            <select name="type" class="form-control" style="width: 100%;">
                ...
                <option value="in" {{ request('type') === 'in' ? 'selected' : '' }}>Material Given to Worker</option>
                <option value="out" {{ request('type') === 'out' ? 'selected' : '' }}>Material Received from Worker</option>
            </select>
        </div>
        ...
        <div style="display: flex; gap: 8px;">
            ...
            <a href="{{ route('stock-register.index') }}" class="btn secondary" style="padding: 9px 16px; font-weight: 500; text-decoration: none;">Reset</a>
        </div>
    </form>
</div>
...
<div class="card" style="border-radius: 12px; overflow: hidden; background: var(--bg-surface); border: 1px solid var(--border-color);">
        ...
        <table class="data-table" style="width: 100%; border-collapse: collapse; text-align: left;">
            <thead>
                <tr style="background: var(--bg-hover); border-bottom: 1px solid var(--border-color); font-size: 13px; color: var(--text-muted);">
                    ...
                    <th style="padding: 14px 16px; font-weight: 700;">ITEM / QUALITY</th>
                    <th style="padding: 14px 16px; font-weight: 700;">CONE COUNT (DETAILS)</th>
                    <th style="padding: 14px 16px; font-weight: 700; text-align: right; color: #f59e0b;">MATERIAL GIVEN TO WORKER</th>
                    <th style="padding: 14px 16px; font-weight: 700; text-align: right; color: #10b981;">MATERIAL RECEIVED FROM WORKER</th>
                    <th style="padding: 14px 16px; font-weight: 700; text-align: right; color: #3b82f6;">BALANCE WITH WORKER</th>
                    <th style="padding: 14px 16px; font-weight: 700;">CONVERSION / NOTES</th>
            ...
            <tbody style="font-size: 14px;">
                ...
                <tr style="border-bottom: 1px solid var(--border-color); transition: background 0.15s;" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
                    ...
                    <td style="padding: 12px 16px; font-weight: 700; color: var(--text-color);">
                        <span style="background: rgba(59, 130, 246, 0.1); color: #3b82f6; padding: 3px 8px; border-radius: 6px; font-size: 13px;">
                            {{ $entry->item_name }}
                        </span>
                    </td>
                    <td style="padding: 12px 16px; color: var(--text-color); font-weight: 500;">
                        {{ $entry->details ?? '-' }}
                    </td>
                    <td style="padding: 12px 16px; text-align: right; font-weight: 700; color: #f59e0b;">
                        @if($entry->qty_in > 0)
                            {{ number_format($entry->qty_in, 3) }} kg
                        @else
                            <span style="color: var(--text-muted); font-weight: 400;">-</span>
                        @endif
                    </td>
                    <td style="padding: 12px 16px; text-align: right; font-weight: 700; color: #10b981;">
                        @if($entry->qty_out > 0)
                            {{ number_format($entry->qty_out, 3) }} kg
                        @else
                            <span style="color: var(--text-muted); font-weight: 400;">-</span>
                        @endif
                    </td>
                    <td style="padding: 12px 16px; text-align: right; font-weight: 800; color: #3b82f6;">
                        {{ number_format(abs($entry->calc_balance ?? 0), 3) }} kg
                    </td>
                ...
                <tr>
                    <td colspan="9" style="padding: 32px; text-align: center; color: var(--text-muted);">
                        <span class="material-symbols-outlined" style="font-size: 48px; opacity: 0.4; display: block; margin: 0 auto 8px auto;">inventory_2</span>
                        No stock ledger records found. Click "Add Stock Entry" to record your yarn stock!
                    </td>
                </tr>
                @endforelse
            </tbody>
            ...
            <tfoot>
                <tr style="background: var(--bg-hover); font-weight: 800; border-top: 2px solid var(--border-color);">
                    ...
                    <td style="padding: 14px 16px; color: #8b5cf6; font-size: 15px; font-weight: 800;">
    ...
    <div style="padding: 16px 20px; display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--border-color); flex-wrap: wrap; gap: 12px; background: var(--bg-surface);">
        <div style="font-size: 13px; color: var(--text-muted); font-weight: 500;">
            Showing <strong>{{ $entries->firstItem() }}</strong> to <strong>{{ $entries->lastItem() }}</strong> of <strong>{{ $entries->total() }}</strong> stock entries
        </div>
...
<!-- High-Contrast Fully Opaque Add / Edit Stock Entry Modal -->
<div id="stockEntryModal" class="modal-backdrop hidden" style="position: fixed; inset: 0; background: rgba(15, 23, 42, 0.7) !important; backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; z-index: 9999;">
    <div class="modal-card" style="background: #ffffff !important; color: #0f172a !important; border-radius: 16px; width: 100%; max-width: 540px; padding: 28px; box-shadow: 0 25px 50px -12px rgba(15, 23, 42, 0.4); border: 1px solid #cbd5e1; position: relative;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 1px solid #f1f5f9; padding-bottom: 14px;">
            <div>
                <h3 id="modalTitle" style="font-size: 18px; font-weight: 800; color: #0f172a !important; margin: 0;">Add Stock Register Entry</h3>
                <p style="font-size: 12px; color: #64748b !important; margin: 4px 0 0 0;">Enter yarn stock issued to worker or received from worker</p>
            </div>
        ...
        <form id="stockEntryForm" autocomplete="off">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 14px;">
                ...
                <div>
                    <label style="display: block; font-size: 12px; font-weight: 700; color: #334155 !important; margin-bottom: 6px; letter-spacing: 0.5px; text-transform: uppercase;">
                        ITEM / QUALITY NAME <span style="color: #ef4444;">*</span>
                    </label>
                    <input type="text" id="itemName" class="form-control" autocomplete="new-password" required style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;" placeholder="e.g. 30 PC, 50 PC">
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 14px;">
                <div>
                    ...
                    <select id="entryType" class="form-control" required style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;">
                        <option value="in">Material Given to Worker</option>
                        <option value="out">Material Received from Worker</option>
                    </select>
                </div>

                <div>
                    <label style="display: block; font-size: 12px; font-weight: 700; color: #334155 !important; margin-bottom: 6px; letter-spacing: 0.5px; text-transform: uppercase;">
                        QUANTITY IN KG <span style="color: #ef4444;">*</span>
                    </label>
                    <input type="number" step="0.001" min="0.001" id="entryQuantity" class="form-control" autocomplete="off" required style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;" placeholder="e.g. 60.000 or 23.000">
                </div>
            </div>

            <div style="margin-bottom: 14px;">
                <label style="display: block; font-size: 12px; font-weight: 700; color: #334155 !important; margin-bottom: 6px; letter-spacing: 0.5px; text-transform: uppercase;">
                    DETAILS / CONE COUNT (NUMBERS ONLY)
                </label>
                <input type="number" min="0" id="entryDetails" class="form-control" autocomplete="off" style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;" placeholder="Enter cone number count (e.g. 170, 203)">
            </div>
            ...
            <div style="margin-bottom: 24px;">
                ...
                <input type="text" id="entryNotes" class="form-control" autocomplete="off" style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;" placeholder="e.g. 30 PC 1 kg Per 14 mts">
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const openModal = (editData = null) => {
        if (editData) {
            ...
            modalTitle.textContent = 'Add Stock Register Entry';
            form.reset();
            entryId.value = '';
            workerId.value = '';
    ...
    document.querySelectorAll('.delete-entry-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`${window.APP_URL}/api/stock-register/${id}`, {
                    method: 'DELETE',
                    headers: {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const payload = {
            ...
            details: entryDetails.value ? parseInt(entryDetails.value) : null,
            conversion_notes: entryNotes.value,
        };

        const url = id ? `${window.APP_URL}/api/stock-register/${id}` : `${window.APP_URL}/api/stock-register`;
        ...
        try {
            ...
            if (res.success) {
                closeModal();
                showToast(res.message || 'Stock entry saved successfully!', 'success');
                setTimeout(() => window.location.reload(), 800);
            } else {
                showToast(res.message || 'Error saving stock entry.', 'error');
            }
        } catch (err) {
            showToast('Network error saving stock entry.', 'error');
        }
    });
    ...
    if (confirmExportBtn) {
        confirmExportBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (format === 'excel') {
                    const headers = ['Date', 'Worker Name', 'Item / Quality', 'Cone Count (Details)', 'Material Given to Worker (kg)', 'Material Received from Worker (kg)', 'Balance with Worker (kg)', 'Conversion / Notes'];
                    ...
                    const totalsRow = ['ALL-TIME OVERALL TOTALS', '', '', `${data.totals.total_cones} Cones`, `${data.totals.total_given} kg`, `${data.totals.total_received} kg`, `${data.totals.balance} kg`, ''];
                    
                    downloadCSV(`Stock_Register_${timestamp}.csv`, headers, rows, totalsRow);
                    showToast('Excel report downloaded successfully!', 'success');
                } else {
                    openPDFReport(
                        'Stock Register Ledger Report',
                        ['Date', 'Worker Name', 'Item / Quality', 'Cone Count', 'Material Given', 'Material Received', 'Worker Balance', 'Notes'],
                        data.entries.map(e => [e.date, e.worker_name, e.item_name, e.details, e.qty_in !== '-' ? e.qty_in + ' kg' : '-', e.qty_out !== '-' ? e.qty_out + ' kg' : '-', e.calc_balance + ' kg', e.notes]),
                        [
                            { label: 'Total Material Given', val: `${data.totals.total_given} kg` },
                            { label: 'Total Material Received', val: `${data.totals.total_received} kg` },
                            { label: 'Material Balance', val: `${data.totals.balance} kg` },
                            { label: 'Total Cones', val: `${data.totals.total_cones} Cones` }
                        ],
                        `Stock_Register_${timestamp}`
                    );
