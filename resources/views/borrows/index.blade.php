...
@section('title', 'Borrow Ledger - Worker Advance & Repayments')
...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; flex-wrap: wrap; gap: 16px;">
    <div>
        <h1 style="font-size: 24px; font-weight: 700; color: var(--text-color); margin: 0; display: flex; align-items: center; gap: 10px;">
            <span class="material-symbols-outlined" style="color: var(--primary-color); font-size: 28px;">account_balance_wallet</span>
            Worker Borrow & Advance Ledger
        </h1>
        <p style="color: var(--text-muted); margin: 4px 0 0 0; font-size: 13px;">Owner ledger tracking money advances given to workers and repayments returned by workers.</p>
    </div>
...
<div class="metrics-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 24px;">
    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Total Given to Worker</span>
    ...
    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Total Returned by Worker</span>
    ...
    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Net Borrow Balance</span>
    ...
    <div class="card metric-card" style="padding: 18px; border-radius: 12px; background: var(--bg-surface); border: 1px solid var(--border-color);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 13px; font-weight: 600; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.5px;">Active Borrowers</span>
            <span class="material-symbols-outlined" style="color: #8b5cf6; background: rgba(139, 92, 246, 0.1); padding: 6px; border-radius: 8px;">engineering</span>
        </div>
        <div style="font-size: 24px; font-weight: 800; color: #8b5cf6; margin-top: 10px;">
            {{ $activeBorrowersCount }} <span style="font-size: 14px; font-weight: 600;">workers</span>
        </div>
    </div>
</div>
...
<div class="card" style="padding: 16px 20px; border-radius: 12px; margin-bottom: 20px; background: var(--bg-surface); border: 1px solid var(--border-color);">
    <form method="GET" action="{{ route('borrows.index') }}" autocomplete="off" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-end;">
        ...
        <div style="flex: 1; min-width: 180px;">
            <label style="display: block; font-size: 12px; font-weight: 600; color: var(--text-muted); margin-bottom: 6px;">SEARCH REMARKS / DETAILS</label>
            <input type="text" name="search" class="form-control" autocomplete="off" value="{{ request('search') }}" placeholder="Search notes or worker name..." style="width: 100%;">
        </div>

        <div style="min-width: 140px;">
            <label style="display: block; font-size: 12px; font-weight: 600; color: var(--text-muted); margin-bottom: 6px;">TRANSACTION TYPE</label>
            <select name="type" class="form-control" style="width: 100%;">
                <option value="">All Types</option>
                <option value="given" {{ request('type') === 'given' ? 'selected' : '' }}>Given to Worker (Out)</option>
                <option value="returned" {{ request('type') === 'returned' ? 'selected' : '' }}>Returned by Worker (In)</option>
            </select>
        </div>
        ...
        <div style="display: flex; gap: 8px;">
            ...
            <a href="{{ route('borrows.index') }}" class="btn secondary" style="padding: 9px 16px; font-weight: 500; text-decoration: none;">Reset</a>
        </div>
    </form>
</div>
...
<div class="card" style="border-radius: 12px; overflow: hidden; background: var(--bg-surface); border: 1px solid var(--border-color);">
        ...
        <table class="data-table" style="width: 100%; border-collapse: collapse; text-align: left;">
            <thead>
                <tr style="background: var(--bg-hover); border-bottom: 1px solid var(--border-color); font-size: 13px; color: var(--text-muted);">
                    ...
                    <th style="padding: 14px 16px; font-weight: 700; text-align: right; color: #f59e0b;">GIVEN TO WORKER (₹)</th>
                    <th style="padding: 14px 16px; font-weight: 700; text-align: right; color: #10b981;">RETURNED FROM WORKER (₹)</th>
                    <th style="padding: 14px 16px; font-weight: 700; text-align: right; color: #3b82f6;">WORKER BORROW BALANCE (₹)</th>
                    <th style="padding: 14px 16px; font-weight: 700;">REMARKS / NOTES</th>
            ...
            <tbody style="font-size: 14px;">
                ...
                <tr style="border-bottom: 1px solid var(--border-color); transition: background 0.15s;" onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background='transparent'">
                    <td style="padding: 12px 16px; font-weight: 600; white-space: nowrap; color: var(--text-color);">
                        {{ \Carbon\Carbon::parse($borrow->entry_date)->format('d/m/Y') }}
                    </td>
                    <td style="padding: 12px 16px; font-weight: 700; color: var(--text-color);">
                        <span style="background: rgba(99, 102, 241, 0.1); color: #6366f1; padding: 4px 10px; border-radius: 6px; font-size: 13px; display: inline-flex; align-items: center; gap: 6px;">
                            <span class="material-symbols-outlined" style="font-size: 16px;">engineering</span>
                            {{ $borrow->worker->name ?? 'Unknown Worker' }}
                        </span>
                    </td>
                    <td style="padding: 12px 16px; text-align: right; font-weight: 700; color: #f59e0b;">
                        @if($borrow->type === 'given')
                            ₹{{ number_format($borrow->amount, 2) }}
                        @else
                            <span style="color: var(--text-muted); font-weight: 400;">-</span>
                        @endif
                    </td>
                    <td style="padding: 12px 16px; text-align: right; font-weight: 700; color: #10b981;">
                        @if($borrow->type === 'returned')
                            ₹{{ number_format($borrow->amount, 2) }}
                        @else
                            <span style="color: var(--text-muted); font-weight: 400;">-</span>
                        @endif
                    ...
                    <td style="padding: 12px 16px; text-align: right; font-weight: 800; color: #3b82f6;">
                        ₹{{ number_format(abs($workerBalances[$borrow->worker_id] ?? 0), 2) }}
                    </td>
                    ...
                    <td style="padding: 12px 16px; text-align: center; white-space: nowrap;">
                        <button class="btn icon-btn edit-entry-btn" 
                            data-id="{{ $borrow->id }}"
                            data-worker-id="{{ $borrow->worker_id }}"
                            data-date="{{ $borrow->entry_date }}"
                            data-type="{{ $borrow->type }}"
                            data-amount="{{ $borrow->amount }}"
                ...
                <tr>
                    <td colspan="7" style="padding: 32px; text-align: center; color: var(--text-muted);">
                        <span class="material-symbols-outlined" style="font-size: 48px; opacity: 0.4; display: block; margin: 0 auto 8px auto;">account_balance_wallet</span>
                        No borrow advance records found. Click "Add Borrow Entry" to record worker advances!
                    </td>
                </tr>
                @endforelse
            </tbody>
            ...
            <tfoot>
                <tr style="background: var(--bg-hover); font-weight: 800; border-top: 2px solid var(--border-color);">
                    <td colspan="2" style="padding: 14px 16px; color: var(--text-color);">ALL-TIME OVERALL TOTALS:</td>
    ...
    <div style="padding: 16px 20px; display: flex; justify-content: space-between; align-items: center; border-top: 1px solid var(--border-color); flex-wrap: wrap; gap: 12px; background: var(--bg-surface);">
        <div style="font-size: 13px; color: var(--text-muted); font-weight: 500;">
            Showing <strong>{{ $borrows->firstItem() }}</strong> to <strong>{{ $borrows->lastItem() }}</strong> of <strong>{{ $borrows->total() }}</strong> borrow entries
        </div>
        <div class="custom-pagination" style="display: flex; gap: 6px; align-items: center;">
            ...
            @else
                <a href="{{ $borrows->previousPageUrl() }}" style="padding: 6px 12px; border-radius: 6px; background: var(--bg-hover); color: var(--text-color); font-size: 13px; text-decoration: none; font-weight: 600;">&laquo; Previous</a>
            @endif
            ...
            @if ($borrows->hasMorePages())
                <a href="{{ $borrows->nextPageUrl() }}" style="padding: 6px 12px; border-radius: 6px; background: var(--bg-hover); color: var(--text-color); font-size: 13px; text-decoration: none; font-weight: 600;">Next &raquo;</a>
            @else
...
<!-- High-Contrast Fully Opaque Add / Edit Borrow Entry Modal -->
<div id="borrowEntryModal" class="modal-backdrop hidden" style="position: fixed; inset: 0; background: rgba(15, 23, 42, 0.7) !important; backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; z-index: 9999;">
    <div class="modal-card" style="background: #ffffff !important; color: #0f172a !important; border-radius: 16px; width: 100%; max-width: 520px; padding: 28px; box-shadow: 0 25px 50px -12px rgba(15, 23, 42, 0.4); border: 1px solid #cbd5e1; position: relative;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 1px solid #f1f5f9; padding-bottom: 14px;">
            <div>
                <h3 id="modalTitle" style="font-size: 18px; font-weight: 800; color: #0f172a !important; margin: 0;">Add Worker Borrow Entry</h3>
                <p style="font-size: 12px; color: #64748b !important; margin: 4px 0 0 0;">Record advance money given to worker or returned by worker</p>
            </div>
        ...
        <form id="borrowEntryForm" autocomplete="off">
            ...
            <div style="margin-bottom: 14px;">
                ...
                <select id="workerId" class="form-control" required style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 14px;">
                ...
                <div>
                    ...
                    <select id="entryType" class="form-control" required style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;">
                        <option value="given">Money Given to Worker (Out / Advance)</option>
                        <option value="returned">Money Returned by Worker (In / Repayment)</option>
                    </select>
                </div>
            </div>
            ...
            <div style="margin-bottom: 14px;">
                ...
                <input type="number" step="0.01" min="0.01" id="entryAmount" class="form-control" autocomplete="off" required style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;" placeholder="Enter amount in ₹ (e.g. 5000 or 1000)">
            </div>
            ...
            <div style="margin-bottom: 24px;">
                ...
                <input type="text" id="entryNotes" class="form-control" autocomplete="off" style="width: 100%; background: #ffffff !important; color: #0f172a !important; border: 1.5px solid #cbd5e1; border-radius: 8px; padding: 10px 14px; font-size: 14px; font-weight: 500;" placeholder="e.g. Festival advance, weekly salary deduction, etc.">
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const modal = document.getElementById('borrowEntryModal');
    const addBtn = document.getElementById('addEntryBtn');
    const closeBtn = document.getElementById('closeModalBtn');
    const cancelBtn = document.getElementById('cancelModalBtn');
    const form = document.getElementById('borrowEntryForm');
    ...
    document.querySelectorAll('.delete-entry-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`${window.APP_URL}/api/borrows/${id}`, {
                    method: 'DELETE',
                    headers: {
                ...
                if (res.success) {
                    showToast('Borrow entry deleted successfully!', 'success');
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const url = id ? `${window.APP_URL}/api/borrows/${id}` : `${window.APP_URL}/api/borrows`;
        ...
        try {
            ...
            if (res.success) {
                closeModal();
                showToast(res.message || 'Borrow entry saved successfully!', 'success');
                setTimeout(() => window.location.reload(), 800);
            } else {
                showToast(res.message || 'Error saving borrow entry.', 'error');
            }
        } catch (err) {
            showToast('Network error saving borrow entry.', 'error');
        }
    });
            ...
            if (format === 'excel') {
                ...
                try {
                    ...
                    openPDFReport(
                        ...
                        data.entries.map(e => [e.date, e.worker_name, e.qty_in !== '-' ? 'Rs ' + e.qty_in : '-', e.qty_out !== '-' ? 'Rs ' + e.qty_out : '-', 'Rs ' + e.calc_balance, e.notes]),
                        [
                            { label: 'Total Money Given', val: `Rs ${data.totals.total_given}` },
                            { label: 'Total Money Returned', val: `Rs ${data.totals.total_returned}` },
                            { label: 'Net Outstanding Balance', val: `Rs ${data.totals.balance}` },
                            { label: 'Active Borrowers', val: `${data.totals.active_borrowers} workers` }
                        ],
